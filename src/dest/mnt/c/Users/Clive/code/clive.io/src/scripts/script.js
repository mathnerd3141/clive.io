...
(function() {
  ...
  $(() => {
    var getScalingFactor, name, orig_dims, props, resize;
    ...
    getScalingFactor = () => {
      ...
      if (curr_dims.width / curr_dims.height > orig_dims.width / orig_dims.height) {
        return curr_dims.width / orig_dims.width;
      } else {
        return curr_dims.height / orig_dims.height;
      }
    };
    resize();
    ...
    if (typeof (props != null ? props.onclicktouch : void 0) === "function") {
      $(window).on("mousedown click touchstart tap touch", (e) => {
        var scale;
        scale = getScalingFactor();
        return props.onclicktouch((e.pageX - parseInt($('canvas#splash').css('margin-left'))) / scale, (e.pageY - parseInt($('canvas#splash').css('margin-top'))) / scale);
      });
    }
